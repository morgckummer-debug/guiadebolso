...
no cabeçalho nem overlay full-screen separado (revisão de 2026-08-08/09,
"FAB absorve busca global do app").
...
cuidado? → volta ao bloco correspondente da leitura, ou abre o seletor de
achados/foca a busca quando fora de um Tema) — sem exigir que o médico
navegue para outro lugar ou lembre do método de cor. Quando a busca não
encontra Tema correspondente, comunica isso com honestidade em vez de
gerar resposta (nunca IA generativa aberta — o valor do produto é a
palavra da própria autora) e guarda a pergunta para revisão editorial.
Ver `RaciocinioFAB` em `03-biblioteca-componentes.md`. Esta camada não é
navegação nem conteúdo de Tema: é a "voz" do produto, sempre acessível no
momento da dúvida.
