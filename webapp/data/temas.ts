...
const TEMAS = [
  ...
  },
  {
    ...
    titulo:'Fêmur curto',
    modulo:'marcadores',
    tags:['fêmur curto','marcador leve','curvas de crescimento','hadlock','biometria fetal','displasia óssea','restrição de crescimento fetal','baixa estatura familiar'],
    essencial:'Fêmur curto isolado é definido como comprimento abaixo do percentil 5 (ou &lt;-2 DP) pela curva usada no laudo. No Brasil, isso pede um cuidado extra: usamos curvas de crescimento construídas em população americana (ex.: Hadlock), constitucionalmente mais alta que a população brasileira — por isso, boa parte dos "fêmures curtos" identificados aqui é só um efeito da curva de referência, não um achado real. Antes de rotular como marcador, confirme a datação e avalie se é proporcional aos outros ossos.',
    erro:'Interpretar todo fêmur curto isolado como marcador de aneuploidia sem considerar que a curva de referência usada (majoritariamente americana, como Hadlock) não reflete a biometria da população brasileira — isso já classifica como "curto" uma proporção grande de bebês brasileiros constitucionalmente normais. Também é erro não confirmar a datação nem avaliar desproporção com os demais ossos longos antes de encaminhar para investigação invasiva.',
    passo:{tipo:'checklist', itens:[
      'Lembre que a curva de referência usada no seu serviço (geralmente baseada em população americana, como Hadlock) tende a superestimar a frequência de "fêmur curto" na população brasileira, constitucionalmente mais baixa — isso já reduz bastante a chance de ser um achado patológico.',
      ...
      'Meça o úmero e avalie a proporção entre os ossos longos — encurtamento desproporcional, associado a tórax estreito, ossos arqueados ou fraturas, sugere displasia óssea, não marcador leve.',
    ...
    explicar:'"O fêmur do seu bebê está um pouco menor do que a média para essa fase da gestação. Isso tem várias explicações possíveis, e a maioria delas não é motivo de preocupação — pode ser só a forma de calcular a data da gravidez, a diferença entre a curva de crescimento usada e a nossa população, ou simplesmente porque vocês são uma família mais baixa. Vamos conferir tudo antes de tirar qualquer conclusão."',
    ...
    aula:{titulo:'Fêmur curto: datação, proporção e o efeito das curvas de crescimento', pendente:true},
    referencias:[
